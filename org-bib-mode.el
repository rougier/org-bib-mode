  ...
  (let ((bibitem (org-bib--get-crossref-info doi)))
    (with-temp-buffer
      ...
      (let* ((author (org-bib--bibtex-get "author"))
             ...
             (booktitle (org-bib--bibtex-get "booktitle"))
             (organization (org-bib--bibtex-get "organization"))
             (journal (org-bib--bibtex-get "journal"))
             (series (org-bib--bibtex-get "series"))
             ...
             (properties
              (concat ":PROPERTIES:\n"
                 (if bibkey (format ":CUSTOM_ID: %s\n" bibkey))
                 (if bibkey (format ":KEY: %s\n" bibkey))
                 (if type (format ":TYPE: %s\n" type))
                 (if author (format ":AUTHOR: %s\n" author))
                 (if editor (format ":EDITOR: %s\n" editor))
                 (if title (format ":TITLE: %s\n" title))
                 (if series (format ":SERIES: %s\n" series))
                 (if booktitle (format ":BOOKTITLE: %s\n" title))
                 (if organization (format ":ORGANIZATION: %s\n" organization))
                 (if year (format ":YEAR: %s\n" year))
                 (if journal (format ":JOURNAL: %s\n" journal))
                 (if volume (format ":VOLUME: %s\n" volume))
                 (if number (format ":NUMBER: %s\n" number))
                 (if doi (format ":DOI: %s\n" doi))
                 (if pdf (format ":FILE: %s\n" filename))
                 ":END:"))
